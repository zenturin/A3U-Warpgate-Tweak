...
/* private _vtol = [];
private _trasportPlanes = FactionGet(all,"vehiclesTransportAir");
{
	if (getNumber (configOf _x >> "vtol") > 0) then {
    	_vtol = append [_x];
	};
} forEach _trasportPlanes;

private _vehFastRope = (FactionGet(all,"vehiclesHelisTransport") + FactionGet(all,"vehiclesHelisLight") + _vtol + FactionGet(all,"vehiclesHelisAttack") + FactionGet(all,"vehiclesHelisLightAttack")); */
