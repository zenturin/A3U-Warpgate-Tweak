...
if (!alive _carried) exitWith {
	[localize  "STR_A3A_revive_carry_header", format [localize "STR_A3A_revive_carry_dead",name _carried]] call A3A_fnc_customHint;
};

if !(_carried getVariable ["incapacitated",false]) exitWith {
	[localize  "STR_A3A_revive_carry_header", format [localize "STR_A3A_revive_no_longer_need_help",name _carried]] call A3A_fnc_customHint;
};

if !(isNull attachedTo _carried) exitWith {
	[localize  "STR_A3A_revive_carry_header", format [localize "STR_A3A_revive_transported",name _carried]] call A3A_fnc_customHint;
};
...
private _action = _carrier addAction [format [(localize "STR_antistasi_actions_release_carry"),name _carried], {{detach _x} forEach (attachedObjects player)},nil,0,false,true,"","(isPlayer _this) and (_this == _this getVariable ['owner',objNull])"];
