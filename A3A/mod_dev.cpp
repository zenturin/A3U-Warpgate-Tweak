#if __has_include("\x\A3A\addons\core\script_component.hpp")
#include "\x\A3A\addons\core\script_component.hpp"
name = QUOTE([Dev] Antistasi Ultimate);
logo = QPATHTOFOLDER(Pictures\antistasi_plus_logo_sw.paa);
logoOver = QPATHTOFOLDER(Pictures\antistasi_plus_logo.paa);
logoSmall = QPATHTOFOLDER(Pictures\antistasi_plus_logo_small.paa);
picture = QPATHTOFOLDER(Pictures\antistasi_plus_overview.paa);
#else
name = "Antistasi Ultimate";
#endif
author = "Antistasi Ultimate Dev Team";
description = "";
tooltip = "Antistasi Ultimate";
tooltipOwned = "Antistasi Ultimate";
overview = "<br/><t align='center' size='0.75'> An Arma 3 persistent multiplayer scenario about guerilla warfare tactics. Fork of Socrate's Antistasi Plus, which is a fork of the Antistasi Community Version. We focused on expanding playable mods, factions and maps. </t><br/><br/>";
actionName = "GitHub";
action = "https://github.com/SilenceIsFatto/A3-Antistasi-Ultimate";
hideName = 0;
hidePicture = 0;
dlcColor[] = {0.1765, 0.6392, 0.6235, 1};
